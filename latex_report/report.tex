...
\textbf{Implementation note (text embeddings):} While the ideal setup uses the aligned EgoVLP/PerceptionEncoder text encoder, some of our local runs use a lightweight alternative to avoid downloading large checkpoints: we encode node descriptions with a small text model and fit a ridge-regression projection into the Step-1 visual embedding space using GT pairs $(\text{description}, v)$. The learned projection is cached (e.g., as \texttt{extension\_data/step3\_text\_to\_visual\_W.npy}) and allows Hungarian matching to operate in the same space as Step 1.
...
We train ActionFormer from scratch for temporal step localization on CaptainCook4D. Table \ref{tab:actionformer_main} summarizes representative hyperparameter experiments.
...
\begin{figure}[h]
\centering
\IfFileExists{../extension_data/plots/matching_analysis.png}{%
  \includegraphics[width=\linewidth]{../extension_data/plots/matching_analysis.png}%
}{%
  \fbox{\parbox{\linewidth}{\centering matching\_analysis.png missing\\(expected at ../extension\_data/plots/matching\_analysis.png)}}%
}
\caption{Matching analysis summary used in Step 3: distribution and summary statistics of cosine similarities produced by Hungarian assignment.}
\label{fig:matching}
\end{figure}
...
\subsection{Effect of Inference Post-processing on ActionFormer}

ActionFormer localization quality is sensitive not only to training, but also to inference post-processing (thresholding, minimum segment length, smoothing, and peak filtering). Table \ref{tab:actionformer_grid} summarizes the top-performing post-processing configurations from our validation sweep (optimized for boundary F1 at $\pm$2s).

\begin{table}[h]
\centering
\caption{ActionFormer inference post-processing sweep (val). Top unique configurations by boundary F1@$\pm$2s.}
\label{tab:actionformer_grid}
{\scriptsize\tighttabcolsep\tighttablerowstretch
\begin{tabular}{lcccc}
\toprule
thr & min\_len & smooth(w) & peak\_dist & F1@2s \\
\midrule
