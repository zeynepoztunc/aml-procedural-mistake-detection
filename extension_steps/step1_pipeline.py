...
# %% [cell 0]
# ------------------------------------------------------------------------------
# Local setup (replaces the original Google Colab bootstrap cells)
#
# This "step1_pipeline" script is the *GT/oracle* localization baseline:
# it uses ground-truth step start/end times from `step_annotations.json` and
# simply pools EgoVLP features inside those time spans to produce per-step
# embeddings for Step 2 (verification).
#
# Defaults (override via env vars if your paths differ):
...
# - OUTPUT_PATH: `extension_data/step_embeddings_gt.pkl`
# ------------------------------------------------------------------------------
...
EGOVLP_FEATURE_DIR = os.environ.get(
    "EGOVLP_FEATURE_DIR", str(REPO_ROOT / "data" / "features" / "egovlp")
)
ANNOTATION_PATH = os.environ.get(
    "ANNOTATION_PATH",
    str(REPO_ROOT / "annotations" / "annotation_json" / "step_annotations.json"),
)
SPLIT_PATH = os.environ.get(
    "SPLIT_PATH", str(REPO_ROOT / "er_annotations" / "recordings_combined_splits.json")
)
OUTPUT_PATH = os.environ.get(
    "OUTPUT_PATH", str(REPO_ROOT / "extension_data" / "step_embeddings_gt.pkl")
)
...
def load_egovlp_features(recording_id):
    # Step 1 (GT pipeline) input: read the per-recording EgoVLP feature sequence from disk.
...
def pool_features(features, start_time, end_time, fps=0.5):
    # Step 1 (GT pipeline) core: mean-pool features inside a GT time window -> one embedding for that step.
    """
...
def extract_step_embeddings_gt(annotations, feature_loader):
    # Step 1 (GT pipeline) output builder: uses GT step boundaries to produce Step-2-compatible records.
    """
