...
def compute_loss(
    *,
    ...
    # Pipeline core (training): masked BCE loss for boundary detection (handles padded timesteps).
...
def train_one_epoch(*, model: torch.nn.Module, loader: DataLoader, device: torch.device, pos_weight: float, optimizer) -> float:
...
def evaluate(*, model: torch.nn.Module, loader: DataLoader, device: torch.device, pos_weight: float, threshold: float = 0.5) -> TrainStats:
    ...
    bin_preds = (preds > threshold).astype(np.float32)

    tp = float(((bin_preds == 1) & (labs == 1)).sum())
    fp = float(((bin_preds == 1) & (labs == 0)).sum())
    fn = float(((bin_preds == 0) & (labs == 1)).sum())
