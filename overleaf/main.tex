...
%%%%%%%%% ABSTRACT
\begin{abstract}
We present a comprehensive study on procedural mistake detection and task verification using the CaptainCook4D dataset. Our work extends the supervised error recognition baselines from CaptainCook4D (V1-MLP, V2-Transformer) with a novel LSTM baseline (V3), and develops a complete task verification pipeline. We reproduce and analyze the original baselines, propose LSTM achieving 58.41\% F1 on the recordings split, and extend the work to recipe-level verification. For temporal action localization, we train ActionFormer from scratch on CaptainCook4D with systematic grid search (81 configurations), achieving F1=9.19\%---a result that, while low in absolute terms, is expected given the domain gap from pre-training datasets and the inherent difficulty of fine-grained cooking step localization. Our extensive hyperparameter analysis reveals a counter-intuitive ``regularization paradox'' where adding dropout/weight decay hurts larger models in data-limited regimes. For task verification, systematic grid search (108 configurations per model) shows all architectures converge to similar performance ($\sim$72\% F1), and GraphSAGE achieves 75.7\% F1 for graph-based classification. We provide detailed analysis of error types and discuss counter-intuitive phenomena observed in low-data regimes.
\end{abstract}
...
\begin{itemize}
    ...
    \item A complete task verification pipeline with task graph matching and GNN classification achieving 75.7\% F1
\end{itemize}
...
\midrule
V1 (MLP) & step & 24.26 & 0.578 & Official Ckpt \\
V2 (Transformer) & step & 55.39 & 0.713 & Official Ckpt \\
\textbf{V3 (LSTM)} & \textbf{step} & \textbf{50.85} & \textbf{0.682} & \textbf{Trained} \\
\midrule
V1 (MLP) & recordings & 50.91 & 0.632 & Official Ckpt \\
V2 (Transformer) & recordings & 39.04 & 0.584 & Official Ckpt \\
...
\begin{table}[h]
\centering
\caption{ActionFormer systematic grid search (81 combinations). Shows top 5 configurations. All models trained for 15 epochs.}
\label{tab:actionformer_grid}
\begin{tabular}{ccccc}
\toprule
Rank & Batch Size & LR & Embd Dim & F1 \\
\midrule
1 & 16 & 5e-4 & 256 & \textbf{0.0919} \\
2 & 8 & 1e-3 & 512 & 0.0901 \\
3 & 4 & 1e-3 & 384 & 0.0876 \\
4 & 4 & 1e-3 & 512 & 0.0867 \\
5 & 4 & 5e-4 & 384 & 0.0849 \\
...
\textbf{Grid Search Configuration:} We tested batch sizes $\in \{4, 8, 16\}$, learning rates $\in \{10^{-3}, 5\cdot10^{-4}, 10^{-4}\}$, embedding dimensions $\in \{256, 384, 512\}$, and dropouts $\in \{0.1, 0.3, 0.5\}$. The best configuration (bs=16, lr=5e-4, embd=256, dropout=0.1) achieves F1=9.19\%, which is lower than our individual experiments with extended training (F1=9.36\% at 100 epochs). This demonstrates that longer training is beneficial for high-dropout configurations.

\subsubsection{Why is ActionFormer Performance So Low?}

The best ActionFormer achieves only F1=11.97\%, which may appear surprisingly low. However, this is \textbf{expected and normal} for several reasons:
...
\textbf{Key Finding - The Regularization Paradox:} We hypothesized that the severe overfitting could be mitigated with stronger regularization. Table \ref{tab:regularization} shows surprising results.

\begin{table}[h]
\centering
\caption{Regularization experiments. Adding regularization hurts the wider architecture but slightly helps the standard architecture.}
...
\begin{itemize}
    \item Simple recipes with distinct steps: similarity scores 0.15-0.25
    \item Complex recipes with similar steps: similarity scores 0.08-0.12
\end{itemize}

The relatively low similarity scores (compared to typical CLIP-style matching) suggest that the EgoVLP feature space, while aligned, has limited discriminative power for fine-grained step matching in cooking domains.
...
\textbf{GraphSAGE} & \textbf{72.5\%} & 76.7\% & \textbf{78.4\%} & \textbf{75.7\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Key Findings:}
\begin{itemize}
    \item GraphSAGE achieves the best F1 (75.7\%), likely due to its inductive learning capabilities that generalize better across different recipe structures
    \item GAT has highest precision (80.4\%) but lower recall
    \item Even simple mean pooling achieves 69.9\% F1, suggesting that graph structure provides limited additional benefit
\end{itemize}
...
\textbf{1. ActionFormer's Low Absolute Performance:} Despite extensive hyperparameter tuning, the best ActionFormer achieves only F1=11.97\%. This is \textbf{expected} given: (a) training from scratch without pre-training on large video datasets, (b) fine-grained cooking steps being inherently hard to localize, and (c) severe overfitting with only 384 training videos. As noted by the teaching assistant, ``the poor performance of ActionFormer is expected'' when training from scratch on this dataset.

\textbf{2. Regularization Paradox:} The most counter-intuitive finding is that adding dropout/weight decay to the wider ActionFormer \textit{hurts} performance (F1 drops from 0.1197 to 0.0772, a 35\% decrease). This contradicts conventional wisdom that regularization helps with overfitting.
...
\textbf{4. High F1 but Low Accuracy:} The best MLP achieves 70.26\% F1 but only 57.0\% accuracy.
...
\textbf{5. Graph Structure Provides Limited Benefit:} Simple mean pooling over graph nodes (69.9\% F1) performs only slightly worse than GNN models (70.5-75.7\% F1).
...
\begin{itemize}
    ...
    \item \textbf{Matching quality:} Low similarity scores (0.08-0.25) in step matching may propagate errors to downstream GNN classification
...
\begin{enumerate}
    ...
    \item \textbf{ActionFormer trained from scratch} achieves F1=9.19\% (grid search) to 9.36\% (extended training), which is expected given the lack of pre-training and small dataset size
    \item \textbf{Regularization hurts in data-limited settings} when models need to learn quickly during early epochs
    \item \textbf{All verification models converge} to similar performance ($\sim$72\% F1) after systematic hyperparameter tuning, suggesting data quantity is the bottleneck
    \item \textbf{GraphSAGE} achieves best task-level classification (F1=75.7\%) using task graph structure
\end{enumerate}
